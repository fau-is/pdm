...
if __name__ == '__main__':
    ...
    csvfile = open("../data/data_v4_converted_select.csv", 'r')
    ...
    for row in spamreader:
        data_set.append(row)
        taggedDocument = gensim.models.doc2vec.TaggedDocument(gensim.utils.to_unicode(str.encode(' '.join(row[3:len(row)]))).split(),[index])
        
        index = index + 1
        documents_context_str.append(taggedDocument)
    
    #train model    
    model = gensim.models.Doc2Vec(documents_context_str, dm=0, vector_size=embedding_size, window=5, min_count=1, alpha=0.025, min_alpha=0.025, worker=12)
    nrEpochs= 1 
    ...
    #save and get model
    model.save('checkpoints/'+str(0)+'_context_attributes_doc2vec_2d'+str(embedding_size)+'.model', sep_limit=2000000000)   
    ...
    #write dataset
    with open("../data/data_hicss_embeded.csv", 'w', newline='') as csvfile:
