...
from process_mining.activity import DCRActivityBase, DCRActivityNest, DCRActivity
from process_mining.conn import DCRConnection, ConnectionTypes, Condition, Milestone
from process_mining.expr import Expression
...
class DCRGraph(object):
    ...
    def create_expressions_dcr_graph(self):
        ...
        for expression in self.dcr_xml_root.iter('expression'):
            ...
            if expression_id is None or expression_str is None:
                continue
        ...
        for constraint in self.dcr_xml_root.iter('constraints'):
            for constraint_type in constraint:
                for connection in constraint_type:
                    ...
                    dcr_connection = DCRConnection.create_connection(source_node, target_node, connection_type, expression)
    ...
    def get_node_by_name(self, activity_name):
        ...
        for activity_id, event_name in self.Mappings.items():
            if event_name == str(activity_name):
