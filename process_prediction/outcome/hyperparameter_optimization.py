...
def train_test_split_for_hyperparameter_optimization(cases):
    ...
    indices_ = [index for index in range(0, len(cases))]

    if args.shuffle:

        if args.seed:
            train_indices_, test_indices_, train_indices, test_indices = train_test_split(indices_, indices_,
        ...
        return train_indices_, test_indices

    else:

        return indices_[:int(len(indices_) * args.split_rate_train_hpo)], \
               indices_[int(len(indices_) * args.split_rate_train_hpo):]
